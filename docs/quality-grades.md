...
| `src/interface/session.py` | A- | Clean, small encrypted-cookie session. Requires strong `SESSION_SECRET_KEY` in production |
...
3. `SESSION_SECRET_KEY` must be configured strongly in production deployments
