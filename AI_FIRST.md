...
Корневой `AGENTS.md` служит короткой автоматически загружаемой точкой входа.
Этот файл содержит полный контракт, а детальная информация по отдельным темам
находится в директории `docs/`. Если этот файл и `docs/` противоречат друг
другу — `docs/` является источником правды.
...
| `AGENTS.md`                 | Автозагружаемая точка входа и обязательный approval gate |
| `AI_FIRST.md`               | Полный контракт работы агента с репозиторием              |
...
Перед любым изменением файлов агент сначала готовит план в чате. План должен
содержать цель, конкретные шаги, точный список создаваемых, изменяемых,
переименовываемых и удаляемых файлов, проверки и известные риски.

После публикации плана агент **останавливается и ждёт явного подтверждения
человека**. Исходная просьба исправить или реализовать что-либо не считается
подтверждением ещё не показанного плана. Подтверждением считаются `++`, `да`,
`делай` или другая однозначная команда, отправленная после плана и списка
файлов.

До подтверждения разрешены только read-only исследование, диагностика и
подготовка плана. Создавать exec-plan или изменять другие файлы до
подтверждения запрещено.

После подтверждения агент создаёт exec-plan — это версионируемый контракт с
человеком и чеклист для самопроверки. Exec-plan также должен быть указан в
предварительно согласованном списке файлов. Для задач, меняющих только
Markdown-документацию, новый exec-plan не требуется: достаточно
подтверждённого chat-плана.

Если в процессе нужен файл вне согласованного списка, агент останавливается,
объясняет изменение scope, публикует новый полный список и ждёт повторного
...
После завершения план переносится в `docs/exec-plans/completed/` с отметками
о выполнении **до финального коммита задачи**. Создавать финальный коммит,
пока план задачи находится в `active/`, запрещено.
...
1. Получить явное подтверждение chat-плана и точного списка файлов.
2. Создать или обновить согласованный план в `docs/exec-plans/`, кроме задач,
   меняющих только Markdown-документацию.
3. Не выходить за подтверждённый scope без повторного согласования.
4. Написать код минимально возможным диффом.
5. Добавить/обновить тесты (покрытие не должно падать).
6. Убедиться, что `just check` проходит без ошибок.
7. Обновить `docs/`, если внесённые изменения делают текущую документацию устаревшей.
8. Проверить, что нефункциональные требования (если затронуты) по-прежнему выполняются.
9. Перенести существующий план задачи из `active/` в `completed/`.
10. Написать подробный коммит-месседж: **Что** сделано? **Почему**? **Как** это было протестировано?
11. Если задача затрагивает публичные контракты (API, схемы, events) — обновить соответствующие спецификации.
...
# 3. Представь в чате план и точный список файлов, затем STOP
# Дождись явного подтверждения человека

# 4. После подтверждения создай exec-plan (кроме Markdown-only задач)
# docs/exec-plans/active/<task-id>.md

# 5. После изменений — полная проверка
just check

# 6. Перенеси существующий exec-plan в completed, затем коммить
# Коммит только по явной команде человека
git commit -m "feat(domain): add X because Y. Tested by Z"
```

---

*Версия контракта: 2.1 | Последнее обновление: 2026-07-25*
